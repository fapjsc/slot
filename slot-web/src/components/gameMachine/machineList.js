    ...
    <div className={classes.root}>
      <Grid container spacing={3} style={{ padding: 10 }}>
        {props.egmList.length > 0
          ? props.egmList.map(({ camera, title, magnification, subHeader, descreption, machineIcon, buttonName }) => (
              <Grid item xs={12} sm={4} md={3} className={classes.grid}>
                <MachineItem camera={camera} title={title} magnification={magnification} subHeader={subHeader} descreption={descreption} machineIcon={machineIcon} buttonName={'開始玩'} />
              </Grid>
            ))
          : null}
        <WolfGame />
